...
import { validateAuthToken, consumeSession, returnSession, markSessionUsed, recordUsage, checkAndIncrementQuota, rollbackQuota } from '@/lib/token-pool.ts';
...
export default {
    ...
    post: {
        ...
        '/generations': async (request: Request) => {
            ...
            // 身份校验 + session 池消耗
            const authResult = validateAuthToken(request.headers.authorization);
            if (!authResult.valid) throw new Error('身份校验失败：无效的 Authorization token');
            const authLabel = authResult.label;
            const quotaCheck = checkAndIncrementQuota(authResult.token, 'videos-cn');
            if (!quotaCheck.allowed) {
                const quotaErr = new Exception([-1, quotaCheck.reason]);
                quotaErr.setHTTPStatusCode(429);
                quotaErr.setData({ quota: quotaCheck.quota?.quotas, used: quotaCheck.quota?.used });
                throw quotaErr;
            }
            const session = consumeSession("cn");
            if (!session) throw new Error('sessionid 池已耗尽，请联系管理员补充');
            const token = session.token;
            const sessionId = session.id;
            const region = session.type;
            ...
            if (isSeedanceModel(model)) {
                ...
                try {
                    ...
                    if (errMsg.includes("-2001") || errMsg.includes("Not enough credits") || errMsg.includes("no relevant benefits")) {
                        logger.warn(`Session ${sessionId.substring(0,16)}... marked as used due to insufficient credits`);
                    }
                    throw genError;
                }
            }
            markSessionUsed(sessionId, authLabel);
            recordUsage(authLabel, {
              authLabel,
        ...
        '/international/generations': async (request: Request) => {
            ...
            if (!authResult.valid) throw new Error('身份校验失败：无效的 Authorization token');
            const authLabel = authResult.label;
            const quotaCheck = checkAndIncrementQuota(authResult.token, 'videos-international');
            if (!quotaCheck.allowed) {
                const quotaErr = new Exception([-1, quotaCheck.reason]);
                quotaErr.setHTTPStatusCode(429);
                quotaErr.setData({ quota: quotaCheck.quota?.quotas, used: quotaCheck.quota?.used });
            ...
            const session = consumeSession("international");
            if (!session) throw new Error('sessionid 池已耗尽，请联系管理员补充');
            const token = session.token;
            const sessionId = session.id;
            const region = session.type;
        ...
        '/international/generationst': async (request: Request) => {
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                try {
                    const result = await fetch(`${request.origin}/v1/videos/international/generations`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (data.code === -2001) {
                        ...
                        if (errMsg.includes('Not enough credits') || errMsg.includes('no relevant benefits') || errMsg.includes('login error')) {
                            lastError = data;
                            if (attempt < maxRetries) {
                                logger.warn(`[/international/generationst] Attempt ${attempt}/${maxRetries} failed with -2001, retrying...`);
                                continue;
                            }
                        }
                    }
                    ...
                    lastError = err;
                    if (attempt < maxRetries) {
                        logger.warn(`[/international/generationst] Attempt ${attempt}/${maxRetries} threw error: ${err.message}, retrying...`);
                        continue;
                    }
                }
            }
