...
async function fetchHighQualityVideoUrl(itemId: string, refreshToken: string): Promise<string | null> {
  try {
    ...
    // 优先用浏览器获取预览视频 URL（无水印）
    try {
      const previewUrl = await browserService.getVideoPreviewUrl(itemId, refreshToken, "international");
      if (previewUrl && !previewUrl.includes("display_watermark_busi_aigc")) {
        logger.info(`通过浏览器获取到无水印预览 URL`);
        return previewUrl;
      } else if (previewUrl) {
        logger.warn(`浏览器获取的 URL 仍含 busi_aigc 水印，回退到 API 方式`);
      }
    } catch (browserErr) {
      logger.warn(`浏览器获取预览 URL 失败: ${(browserErr as Error).message}`);
    }
...
export async function generateSeedanceVideo(
  ...
  if (ret !== undefined && Number(ret) !== 0) {
    ...
    // Check for -2001 insufficient credits error
    if (Number(ret) === -2001 || errmsg.includes("Not enough credits") || errmsg.includes("no relevant benefits")) {
      logger.warn(`Jimeng API returned insufficient credits (ret=${ret}), session should be marked as used`);
    }
